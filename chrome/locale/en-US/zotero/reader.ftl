...
reader-prompt-store-in-pdf-title = Store Annotations in PDF
reader-prompt-store-in-pdf-text = Existing Zotero annotations will be moved into this PDF file and removed from the Zotero database. Continue?
...
annotation-storage-conflict-window = Resolve Annotation Conflicts
    .title = Resolve Annotation Conflicts
annotation-storage-conflict-description = The PDF file and Zotero contain different versions of these annotations. Choose which version to keep for each annotation. Cancel opens the PDF read-only without changing either version.
annotation-storage-conflict-item = Conflict { $index }: { $type } annotation, page { $page }
annotation-storage-conflict-use-pdf =
    .label = Use PDF
annotation-storage-conflict-use-zotero =
    .label = Use Zotero
